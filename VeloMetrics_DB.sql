...
select productkey, customerkey, count(*) as count
from fact_sales_union
group by productkey, customerkey
having count(*) > 1;

select count(*) from fact_internet_sales;
    

-- kpi on No. products and customers and regions
select count(*) from dimproduct;
select count(*) from dimcustomer;
select SalesTerritoryRegion, sum(Salesamount) from DimSalesTerritory join fact_sales_union;
